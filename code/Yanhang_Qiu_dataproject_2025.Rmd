...
# Introduction to the NHANES Dataset
...
```{r 1.2 Variable Definitions, fig.height=3, fig.width=8, message=FALSE, warning=FALSE}
...
```{r 2.1 Continuous Age Distribution, fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
...
```{r 2.1 Categorial Age Distribution, fig.height=3, fig.width=4, message=FALSE, warning=FALSE}
library(dplyr)
library(ggplot2)
...
## Handling Missingness in Marital Status
...
```{r 2.2 Recoding of Missing Value, message=FALSE, warning=FALSE}
...
```{r 2.3 Distribution of Self-rated Health, fig.height=3, fig.width=4, message=FALSE, warning=FALSE}
...
```{r 2.3 Analysis of Missing Data Patterns, message=FALSE, warning=FALSE}
...
```{r 3.1 Recoding of Diabetes Status1, fig.height=4, message=FALSE, warning=FALSE}
library(dplyr)
library(binom)
...
```{r 3.1 Recoding of Diabetes Status2, fig.height=4, message=FALSE, warning=FALSE}
library(binom)
...
```{r 3.2 Assessing the Association Using a Chi-Square Test, message=FALSE, warning=FALSE}
...
```{r 3.2 Analysis of Missing Data Patterns, message=FALSE, warning=FALSE}
...
## Handling Missingness in Working Hours
...
```{r 4.1 Recoding of Missing Value, message=FALSE, warning=FALSE}
...
```{r  4.2 Recoding of Working Hours, message=FALSE, warning=FALSE}
...
## Mosaic Plot of Working Hours × Health Status
...
## Two-Proportion Statistical Tests
...
```{r 4.4 Chi-Square Test of Association, message=FALSE, warning=FALSE }
...
## Distribution of Blood Mercury by Gender

Describe and plot the distribution of blood mercury level in men and women.

```{r 5.1 Distribution of Hg by Gender, message=FALSE, warning=FALSE  }
...
```{r 5.1 Blood Mercury Level classification, message=FALSE, warning=FALSE}
...
## Comparison Using Parametric and Non-Parametric Tests
...
```{r 5.2 Para and Non-Para Test preparation, message=FALSE, warning=FALSE}
...
```{r 5.2 Para and Non-Para Test, message=FALSE, warning=FALSE}
...
## Correlation Analysis Between BMI and HDL
...
```{r analysis of linearity, fig.width=7, fig.height=9, fig.align='center', out.width='95%', message=FALSE, warning=FALSE}
layout(matrix(c(1, 1,
...
```{r two correlation methods, fig.width=7, fig.height=9, fig.align='center', out.width='95%', message=FALSE, warning=FALSE}
...
```{r SLR, fig.width=7, fig.height=9, fig.align='center', out.width='95%', message=FALSE, warning=FALSE}
...
```{r 6.2 Multiple Linear Regression1, message=FALSE, warning=FALSE}
...
```{r 6.2 Multiple Linear Regression2, message=FALSE, warning=FALSE}
...
```{r 6.2 Multiple Linear Regression3, message=FALSE, warning=FALSE}
summary(model_controlled)
...
```{r 6.3 Converting Variable Types and Excluding Incomplete Observations }
...
```{r 6.3 Scatter_dots,fig.width=7, fig.height=8,fig.align='center'}
...
plot(data_mod_lm$bmi,    data_mod_lm$hdl, main="HDD vs BMI")
...
-   Correlations among continuous variables were modest (all \|r\|\< 0.4), and no influential outlines remained after cleaning.
...
```{r 6.3 Specification of the Full Model (Main Effects and Interactions) }
library(dplyr)
...
```{r 6.3 model with one interaction term, message=FALSE, warning=FALSE}
library(car)
vif(model_one_inter)
...
### Step 5. Model Selection using AIC
...
```{r 6.3 Checking 5 Assumptions, fig.width=7, fig.height=8, fig.align='center', warning=FALSE, message=FALSE}
...
## Occupational Exposure: Cancer and Pollutants at Work
...
```{r 7.4 categorical variables vs outcome,fig.width=7,fig.height=8,fig.align='center',warning=FALSE}
...
```{r 7.4 Exploratory Data Analysis3, warning=FALSE}
...
```{r 7.4 Model Diagnostics and Assumption Checking,warning=FALSE, message=FALSE,fig.width=7, fig.height=5, fig.align='center'}
...
• The Standardized Residuals plot showed that most residuals were small, and none exceeded the \$\\pm3\$ threshold, suggesting no major outliers.
