...
plot_age_cate <-ggplot(data, aes(x=factor(age_group))) + geom_bar(fill = "#BFD7EA") + 
...
p1 <- data %>%
  ...
  ggplot(aes(x = srhgnrl_f, group = 1)) +
  geom_bar(aes(y = after_stat(prop)), fill = "#F6BD60") +
...
vars <- c("male", "age_group", "increl")

chisq_summary <- lapply(vars, function(v) {
  tbl <- table(self_health[[v]], self_health$missing_self)
...
Chi-square tests showed that missingness in the self-rated health variable was not random, but significantly related to sex (χ² = 13.52, p \< 0.001), age group (χ² = 40.49, p \< 0.001), and family income (χ² = 23.96, p \< 0.001). Men, older participants, and those with lower income were more likely to have missing values. This indicates that the missing-data mechanism is unlikely to be MCAR, and complete-case analyses may bias the results.
...
After computing the adjusted GVIF for each predictor, all values were well below commonly used thresholds (GVIF\^(1/(2\*df)) \< 2-5), indicating that multicollinearity is not a concern in this model.
