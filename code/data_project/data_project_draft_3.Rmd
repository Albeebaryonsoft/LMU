...
## Characteristics of NHANE Study
...
categories <- list(
  ...
  lifestyle =c("alc_lft", "drnkprd_prv12mo", "cigsprd_prv30d", "sleep_dur", "sleep_probl",
               ...
               "cannab_ever","harddrg_ever","jobstat_lwk"),
...
# Age, Marital Status and Self-Rated Health
...
```{r 2.1 continuous age distribution, fig.height=4, fig.width=8, message=FALSE, warning=FALSE}
...
```{r 2.1 categorial age distribution, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
library(dplyr)
library(ggplot2)
...
## Missing Value in Marital Status
...
## Concerns about Self-Rated Health
...
## Association between Diabetes and Ethnicity
...
# Working Hours and Self-Evaluated Health

## Missing Value in Working Hours
...
According to the mosaic plot, the distribution of self-rated health levels is similar across the two working-hour groups (\< 40 vs. $\ge$ 40 hours).

## Statistical Test for Two Proportions
...
he chi-square test indicated no statistically significant association between weekly working-hour categories and self-rated health ($\chi^2 = 3.25$, $df = 4$,\$ p = 0.517\$). This aligns with the mosaic plot, which also showed very similar health distributions across the two working-hour groups.

# Blood Mercury Level and Gender

## Distribution of Blood Mercury Level
...
## Parametric and Non-Parametric Test for Two Samples
...
## Correlation between BMI and HDL
...
```{r 6.1 pearson and spearman correlation, fig.height=9, fig.width=12, message=FALSE, warning=FALSE}
...
The scatterplot shows that HDL generally decreases as BMI increases, but the points are widely dispersed, indicating that the linear trend is present but weak. The boxplots also reveal several outliers. To obtain more robust and stable results, both Pearson’s r and Spearman’s $\rho$ were therefore examined.

The Pearson correlation between BMI and HDL was $–0.28$ ($95%$ CI: $–0.31$ to $–0.25$), indicating a weak negative linear association. The relationship was highly statistically significant ($t = –19.1$, $df = 4364$, $p<2.2\times10^{-16}$). In addition, the Spearman rank correlation was –0.33 ($p<2.2\times10^{-16}$), suggesting a slightly stronger monotonic decreasing pattern. Together, both measures support a statistically significant but relatively weak negative association between BMI and HDL.
...
Try to find a better model to predict HDL by including more covariates.
...
Each 1-unit increase in BMI was associated with a 0.02446 mmol/L decrease in HDL ($\beta = -0.02446$, $p<2\times10^{-16}$). BMI was one of the strongest negative predictors.
