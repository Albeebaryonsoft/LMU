...
unique(summary_table[, c("variable", "category")])

library(dplyr)
library(purrr)

# -------- 1️⃣ 配置优先变量策略 --------
USE_SBP <- TRUE      # 保留 rr_sys 而去掉 rr_dia
USE_CIGS_CONT <- TRUE # 保留 cigsprd_prv30d（连续），删除 smokstat

# -------- 2️⃣ 派生 PHQ 总分 --------
phq_items <- paste0("phq", 1:9)
if (all(phq_items %in% names(data))) {
  data <- data %>%
    mutate(phq_total = rowSums(across(all_of(phq_items)), na.rm = TRUE))
}

# -------- 3️⃣ 统一排除清单 --------
drop_vars <- c(
  # ID / 派生
  "seqn", "age.cat", "age_bin", "bmi_cat",
  # 因变量
  "hdl", "logHdl",
  # 重复二选一（稍后根据配置再补）
  # 粗糙或短期指标
  "srphbad_prv30d", "srmhbad_prv30d", "adlimp_prv30d", "jobstat_lwk",
  # 疾病类冗余
  "asthma_now", "livdis_now", "ovrwght_ever", "hivpos",
  # 量表条目
  phq_items,
  # 环境暴露（扩展分析时再加）
  "cd", "pb", "hg",
  # 其他次要项
  "rel_asthma", "alc_lft", "smoke_bin"
)

# 根据配置确定去掉哪一方
if (USE_SBP) {
  drop_vars <- c(drop_vars, "rr_dia")  # 留 rr_sys
} else {
  drop_vars <- c(drop_vars, "rr_sys")
}

if (USE_CIGS_CONT) {
  drop_vars <- c(drop_vars, "smokstat")  # 留 cigsprd_prv30d
} else {
  drop_vars <- c(drop_vars, "cigsprd_prv30d")
}

# -------- 4️⃣ 在 summary_table 打标记 --------
summary_table <- summary_table %>%
  mutate(keep_final = if_else(variable %in% drop_vars, FALSE, TRUE))

# -------- 5️⃣ 提取保留变量并生成模型变量集 --------
model_vars <- summary_table %>%
  filter(keep_final) %>%
  group_split(category) %>%
  set_names(map_chr(summary_table %>% filter(keep_final) %>% group_keys(), "category")) %>%
  map(~ .x$variable)

# -------- 6️⃣ 展开所有可建模变量 --------
vars_model <- unique(unlist(model_vars))

# -------- 7️⃣ 创建建模用数据集 --------
data_model <- data %>%
  select(all_of(vars_model), hdl, logHdl, phq_total) %>%   # 保留 HDL 作为目标
  relocate(hdl, .before = 1)

# -------- 8️⃣ 输出汇总信息 --------
cat("✅ 可用于建模的变量数量：", length(vars_model), "\n")
cat("✅ 包含的类别：", paste(names(model_vars), collapse = ", "), "\n\n")

# 每类展示前几个变量
for (cat_name in names(model_vars)) {
  cat("###", cat_name, "\n")
  cat(paste(model_vars[[cat_name]], collapse = ", "), "\n\n")
}

# -------- 9️⃣ 缺失率快速检查（方便后续筛除高缺失变量） --------
missing_summary <- tibble(variable = vars_model) %>%
  mutate(
    missing_rate = map_dbl(variable, ~ mean(is.na(data[[.x]])) * 100),
...
age, educ, ethnic, increl, martlst, male, martlst
bmi, height, rr_dia, weight
frzfood_prvmo，milk_month，rdyfood_prvmo
