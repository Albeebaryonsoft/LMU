...
For linear regression, the EDA should check whether each continuous predictor shows an approximately linear relationship with the continuous outcome, examine correlations between predictors to detect multicollinearity, and identify major skewness or extreme outliers in each predictor variable that could distort estimation.
...
HDL was unimodal with mild right-skewness and required no transformation. Continuous predictors showed plausible ranges and generally linear relationships with HDL. 
...
The full model explained about 26% of the variability in HDL. Significant predictors were sex, BMI, systolic blood pressure, cigarette consumption, and log_cd.Among categorical variables, only Black ethnicity differed significantly from the reference group.Education and income categories were not associated with HDL.All GVIF values were below 2, indicating no multicollinearity concerns.
...
model_full <- lm(hdl ~ age + male + ethnic + educ + increl +
                   bmi + rr_sys +
                   sleep_dur + drnkprd_prv12mo + cigsprd_prv30d + phq2 + log_cd + log_hg,
...
Using backward AIC selection, the final model retained seven predictors—age, sex, ethnicity, BMI, systolic blood pressure, cigarette consumption, and log-transformed cadmium—yielding the minimum AIC (AIC=-1014.83).
...
model_final <- lm(hdl ~ age + male + ethnic + bmi + rr_sys + cigsprd_prv30d + log_cd,
                  data = data_mod_lm)
...
The Durbin–Watson statistic indicated no evidence of autocorrelation (p=0.144 > 0.05), which is expected given the cross-sectional structure of the NHANES dataset.The independence assumption is therefore satisfied.
...
train <- data_mod_lm[train_index, ]
test  <- data_mod_lm[-train_index, ]
...
In our analysis, the mean squared error (MSE) obtained by applying the full-data model to the test set ( 0.137) was slightly lower than the MSE obtained using the training-only fitted model (0.145).
...
The final model for predicting HDL levels included the covariates age, sex, ethnicity, BMI, systolic blood pressure (rr_sys), recent cigarette consumption (cigsprd_prv30d), and log-transformed blood cadmium (log_cd).

Adjusting for all other predictors:

• Age

Each additional year of age was associated with a 0.00193 mmol/L increase in HDL (β = 0.00193, p = 0.115).
The association was small and not statistically significant.

• Sex

Males had 0.153 mmol/L higher HDL compared with females (β = 0.15345, p < 0.001).
This remained one of the strongest predictors in the model.

• Ethnicity

Relative to the Hispanic reference group:

White: +0.0839 mmol/L (p = 0.113)

Black: +0.2303 mmol/L (p < 0.001)

Other/Mixed: +0.1275 mmol/L (p = 0.0647)

Black ethnicity showed a clear positive association with HDL; the other categories did not reach significance.

• BMI

Each 1-unit increase in BMI was associated with a 0.0245 mmol/L decrease in HDL (β = –0.02445, p < 0.001).
BMI was the strongest predictor, showing a robust inverse relationship.

• Systolic Blood Pressure (rr_sys)

A 1 mmHg increase in systolic blood pressure corresponded to a 0.00264 mmol/L increase in HDL (β = 0.00264, p = 0.011).
The effect was statistically significant but small.

• Recent Cigarette Consumption (cigsprd_prv30d)

Each additional cigarette consumed in the past 30 days was associated with a 0.00546 mmol/L decrease in HDL (β = –0.00546, p = 0.0067).
This was a meaningful and statistically significant negative effect.

• Log-transformed cadmium (log_cd)

A 1-unit increase in log_cd was associated with a 0.0507 mmol/L increase in HDL (β = 0.05073, p = 0.045).
This association was statistically significant and improved model fit.
...
Using an exact binomial method, the 95% confidence interval ranged from 8.0% to 9.6%. 
...
```
Among individuals exposed to workplace pollutants for a longer period, the prevalence of the outcome was 7.6%.
Among those without such exposure, the prevalence was 6.9%.A two-sample test for equality of proportions showed no statistically significant difference between the two groups (χ² = 0.67, p = 0.414).
...
Based on epidemiological relevance, data availability, and prior literature, we initially selected a set of demographic, socioeconomic, lifestyle, and biomarker variables—including age, sex, ethnicity, education, BMI, smoking, alcohol consumption, blood metal exposures (cadmium, mercury, lead), self-rated health, and cancer history—for further analysis.
...
data_mod_glm <- data_sub_glm %>%
  mutate(
    ...
    educ = factor(educ,
                  levels = c(1,2,3,4,5),
                  labels = c("edu1","edu2", "edu3", "edu4" ,"edu5")),
...
For logistic regression, the EDA should check the event rate to detect outcome imbalance, evaluate whether each predictor shows a monotonic change in outcome probability across its range, ensure each categorical predictor has adequate sample sizes in all categories, and consider log-transforming continuous predictors that are heavily skewed and may undermine stable estimation.
```{r}


tab_y   <- table(data_mod_glm$cancer_ever, useNA = "ifany")
prop_y  <- prop.table(tab_y)

cbind( n = tab_y,prop = round(prop_y, 3))


par(mfrow = c(2,3))
for(v in c("cd","hg","pb","age","bmi")){
  hist(data_mod_glm[[v]], main = v, xlab = v)
}
par(mfrow = c(1,1))


table(data_mod_glm$ethnic,   useNA = "ifany")
table(data_mod_glm$educ,     useNA = "ifany")
table(data_mod_glm$smokstat, useNA = "ifany")


by(
  data_mod_glm$cancer_ever,
  data_mod_glm$smokstat,
  function(x) mean(x == "Cancer", na.rm = TRUE)
)


par(mfrow=c(2,3))
for(v in c("age","bmi","cd","hg","pb")){
  boxplot(data_mod_glm[[v]] ~ data_mod_glm$cancer_ever,
          main=v, xlab="cancer_ever", ylab=v)
}
par(mfrow=c(1,1))

cor(data_mod_glm[, c("cd","hg","pb")], use="pair")


table(data_mod_glm$ethnic, data_mod_glm$cancer_ever)
table(data_mod_glm$educ,   data_mod_glm$cancer_ever)
table(data_mod_glm$smokstat, data_mod_glm$cancer_ever)

```

The EDA showed that the data are suitable for logistic regression. Metal exposures were strongly right-skewed, so log transformation is appropriate. Age differed clearly between cancer and non-cancer groups and will be an important confounder, while BMI and metal levels showed only small differences. All categorical variables had enough observations in each outcome group, with no sparse cells or separation. There was no relevant collinearity among the metal exposures. Overall, the EDA revealed no problems, and the dataset is ready for modeling.
...
Step 4. Model Selection - AIC
```{r}
fit_step <- step(model_glm_full, direction = "both")
summary(fit_step)
