...
a) What are important characteristics of the NHANES study (study design, target population, study objectives, study period, ...)? 
      ...
      - sampling methods: A four-stage stratified probability cluster sample. <br>
       
b) What general categories (such as "demographics", "lab results") can the variables in the dataset be sorted into? <br>
...
categories <- list(
  ...
  self_rated_health =c("srhgnrl","srphbad_prv30d","srmhbad_prv30d","adlimp_prv30d")
)
...
```
<br>
2. Using the subsample you have chosen, describe the US population with regards to <br>
...
```
<br>The age distribution in the sample shows limited representativeness of the U.S. population, displaying an approximately uniform count across all age intervals. Despite the complex four-stage sampling design, potential biases may still arise — for example, due to substantial differences in age composition across districts, or because older participants tend to be less willing to respond compared to younger individuals.<br>
...
data <- data %>%
  mutate(martlst = ifelse(is.na(martlst), 77, martlst))
...
by_gender
chisq.test(table(self_health$male, self_health$missing_self))

by_age
chisq.test(table(self_health$age_group, self_health$missing_self))

by_increl
chisq.test(table(self_health$increl, self_health$missing_self), simulate.p.value = TRUE)

by_dep
chisq.test(table(self_health$dep_group, self_health$missing_self))
# chisq.test(table(self_health$increl, self_health$missing_self))

by_hiv
chisq.test(table(self_health$hivpos, self_health$missing_self))

```

Chi-square tests confirmed that, except for HIV status, all other four variables were significantly associated with missingness.Therefore, the missing data cannot be considered "Missing Completely At Random (MCAR)".Deleting observations with missing values could introduce selection bias, and thus, appropriate methods such as multiple imputation or inclusion of missing indicators should be applied to handle these data appropriately.<br>
